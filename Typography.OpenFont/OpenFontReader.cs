...
namespace Typography.OpenFont
{
    ...
    public class OpenFontReader
    {
        ...
        public Typeface Read(string fontFile)
        {
            Typeface tf = null;

            using (System.IO.FileStream fs = System.IO.File.OpenRead(fontFile))
            {
                tf = this.Read(fs);
            }
        ...
        public Typeface Read(Stream stream, int streamStartOffset = 0, ReadFlags readFlags = ReadFlags.Full)
        {
            ...
            if (Read(typeface, null, stream, streamStartOffset, readFlags))
            {
                if (stream is FileStream)
                {
                    ...
                    typeface.FilePath = fs.Name;
                    typeface.Filename = System.IO.Path.GetFileName(fs.Name);
                }
